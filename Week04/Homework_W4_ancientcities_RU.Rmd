...
setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Cognitive Science/6th_semester/spatial_analytics/cds-spatial/Week04")
```
...
head(cities_sf3035)
st_crs(cities_sf3035) ##seems about right
```


### Question: 
*1. What are the measurement units of the `cities_sf3035` object?*
Since the projection is 2D, my qualified guess would be that the units are meters. 
...
*I really cant make tmap work, so I unfortunately can't plot this* 
...
#download.file("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json", "~/Library/Mobile Documents/com~apple~CloudDocs/Cognitive Science/6th_semester/spatial_analytics/cds-spatial/Week04/inscriptions.json")


# Load it into R from wherever you put it, and convert into a tibble
list_json <- jsonlite::fromJSON("~/Library/Mobile Documents/com~apple~CloudDocs/Cognitive Science/6th_semester/spatial_analytics/cds-spatial/Week04/inscriptions.json")
...
leaflet(i_large) %>% addTiles() %>% addMarkers(lng=i_sm$longitude,lat=i_sm$latitude)
```
...
Well, I just accidentally plotted the entire large dataset and it looked like the data was from the Mediterranean sea-ish. Futhermore, the plotted subset of the data reveals Italy. 
...
Well, st_crs(i) reveals that the inscriptions have no CRS, so that's why we give it a CRS and I suppose we give it a CRS that matches something else we wanna plot/merge the inscriptions with. Why specifically EPSG 4326, that is a great question, that I still have not figured out how to answer, since this whole entire CRS-thing is not clear to me (!).
...
insc_sf3035 <- st_transform(cities_5km)
...
plot(st_geometry(insc_ch), border='red', lwd=2, add = TRUE)
plot(st_geometryc(c_buff), add = TRUE)
...
# I suppose a division of the two (rural and cities) gives the ratio
length(insc_urban$Country)/length(insc_rural$Country)
```
