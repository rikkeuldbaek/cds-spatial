...
setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Cognitive Science/6th semester/spatial_analytics/cds-spatial/Week04")
...
monuments<- read_sf("/Users/rikkeuldbaek/Library/Mobile Documents/com~apple~CloudDocs/Cognitive Science/6th semester/spatial_analytics/cds-spatial/Week04/fortidsminder/anlaeg_all_25832.shp")
...
library(sp)
library(raster) #for the getData function

municipalities <- getData("GADM", country = "DNK", level =2)

municipalities_25832 <- municipalities %>% 
  st_as_sf(municipalities) %>%  #make sf object
  st_transform(crs = 25832) #transform the crs of the sf object
  
municipalities_25832 # yes? :)


# Plot the monuments on top of the municipalities
plot(st_geometry(municipalities_25832))
plot(viking, add = TRUE, pch = 16, col = "red") #okay this looks pretty terrible :)
```
...
plot(st_geometry(municipalities_25832))
plot(least$geometry, add = TRUE, pch = 16, col = "red") #okay this looks pretty terrible :)

#### ADELAS PLOT, hmm not really pwetty :/
...
# Buffer the points
nb_buf <- st_buffer(nb_crs, dist = 10000)
...
viking_clip <- st_intersection(nb_buf)
parks_clip <- st_intersection(nb_buf)
...
nearest10index <- which(sorted == 10) # which distances are smaller than the 10th smallest distance
?which

# Identify the viking monuments that are the nearest, what kinds of monuments are they?
viking_clip[d,]$anlaegsbet
