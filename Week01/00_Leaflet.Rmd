...
leaflet() %>%
  ...
  addAwesomeMarkers(lng = c(-3, 23, 11),
                    lat = c(52, 53, 49), 
                    popup = popup) #the labels of the specified addAwesomeMarkers
...
*more interactive, and adding a menu for the user to click and select different views of the map*
...
#Set the location and zoom level
leaflet() %>% 
  setView(9.5155848,55.9396761, zoom = 7) %>%
  addTiles()  # checking I am in the right area
...
places <- read_sheet("https://docs.google.com/spreadsheets/d/1PlxsPElZML8LZKyXbqdAYeQCDIvDps2McZx1cTVWSzI/edit#gid=1275868348",
                     range = "SA2022",
                     col_types = "cccnncnc") #make sure to get characters as characters and numbers as numbers :) 
glimpse(places)
```
...
DKmap %>% 
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste0("Description: ", places$Description, "<br>",
                            "Placename: ", places$Placename,  "<br>",
                            "Type: ", places$Type), 
             clusterOptions = markerClusterOptions()) #note verdens navle er her også, men øhm det er bare fra noget tidligere i scriptet
