...
setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Cognitive Science/6th semester/spatial_analytics/cds-spatial/Week02")
```
...
# Load the sf package
#pacman::p_load(sf, GDAL, GEOS, PROJ.4)
...
- feature = how many shapes are in each line (rows in the dataset) 
Forests: 32 features (rows)
Parks: 47 festures (rows)
playgrounds : 51 features (rows)
shelters: 19 (rows)

- layers = layer is just the object (i.e., forrest, park, playground and shelter) 

2. What is the CRS value in these objects?
- Forests: PROJCRS["ETRS89 / UTM zone 32N" and ID["EPSG",25832]] 
- Parks: PROJCRS["ETRS(datum)89 / UTM zone 32N" and ID["EPSG",25832]] 
- Playgrounds: None *you have to define this later, so it is consistent with the other layers* 
- Shelters: PROJCRS["ETRS89 / UTM zone 32N" and ID["EPSG",25832]] 
...
hist(areas, main = "Areas in Forests")

areas <- unclass(areas)

<<<<<<< HEAD:Week02/02_ReadSpatialDataDK_exercise_RU.Rmd
...
plot(big_forests["navn"], main = "Name")
...
class(elevation) #raster class
class(aarhus) #raster class

# Identify how many layers each object has

nlayers(elevation) #1 layer
...
- The elevation raster has 876 columns and 396 rows.

2. What is the resolution of `aarhus` orthophoto? How many layers does it contain and what do they represent?
- the resolution of "aarhus" is: 1, 1  (x, y), and it has 5 layers. 

<<<<<<< HEAD:Week02/02_ReadSpatialDataDK_exercise_RU.Rmd
Now you've learned how to read in single and multi-band rasters. You should have noticed, based on the `nlayers()` function, that the `elevation` object has a single layer and the `aarhus` object has four. *???? 5 ????*
=======
...
ncell(aarhus) #11115556 cells
ncell(elevation) # 346896 cells
```
...
- I suppose these units are raster cells, and the reason for Aarhus city having more cells could be the fact that it contains 5 layers, while the "elevation" object contains 1 layer and is only partially filled with raster cells (evident from plotting)?. 
...
plotRGB(aarhus, stretch= "lin");plotRGB(aarhus, add = TRUE) #lin makes it bright #plotting on top of each other makes no sense
plotRGB(aarhus, stretch= "lin") #this version is better
...
st_crs(forests) #"EPSG",25832
st_crs(playgrounds) #none

# Assign the CRS to playgrounds
crs_1 <- st_crs(forests) #giving playground the same crs as forrest
...
### setting playgrounds crs to Aaarhus
crs(playgrounds) <- 4326 #the aarhus csr
...
```{r plot, eval=FALSE}
#projecting aarhus on playgrounds
arhus25832 <- projectRaster(aarhus, crs =25832,  method = "ngb")

st_crs(arrhus25832) #should be the same as the playground object
st_crs(playgrounds_crs) #crs: 25832

# Plot aarhus and playgrounds (run both lines together)
# Do you see the playgrounds?
plotRGB(arrhus25832, alpha = 200);plot(playgrounds_crs, add = TRUE) #unable to make this work


st_crs(playgrounds_crs) == crs(arrhus25832)

crs(aarhus)
### transform and makeplaygrounds match the CRS of aarhus
#playgrounds_crs <- st_transform(playgrounds, crs = st_crs(aarhus, asText = TRUE))


# Look at the CRS to see if they match --> they do
st_crs(aarhus)
st_crs(playgrounds_crs)
...
pacman::p_load(tmap) ##unable to load this package.. 
library(tmap)
tm_shape(aarhus) + 
    tm_rgb() + 
    tm_shape(parks) + 
