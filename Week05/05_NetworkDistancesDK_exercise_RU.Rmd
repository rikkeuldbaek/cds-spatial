...
pacman::p_load(Rcpp, sf, raster,tidyverse)
...
home <- mb_geocode("Grønbjergvej 17, Ørnhøj, 6973")
...
```{r nobel-drive}
nobel_iso_drive <- mb_isochrone(nobel,# makes a polygon around this point
                                 profile = "walking", # could also be "walking", "biking"
                                 time = c(4, 8, 12)) # how far can u get from the nobel point in 4, 8 and 12 minutes, but note this is just DISTANCE! no traffic information :)


nobel_iso_drive # gives 3 polygons, we're plotting it below
```
...
mapbox_map %>%
  addPolylines(data = route_dir, 
               popup = paste0(
                 "Distance (km): ",
                 round(route_dir$distance, 1), 
                 "<br/>Time (minutes): ",
                 round(route_dir$duration, 1)
               )) #this is only distance note :))
...
home_iso_drive <- mb_isochrone(home,# makes a polygon around this point
                                 profile = "driving", # could also be "walking", "biking"
                                 time = c(10,20,30)) # how far can you get in 10, 20, 30 minutes 
...
leaflet() %>%
  ...
  addLegend(labels = c(4, 8, 12),
            colors = colors,
            title = "Drive-time<br/>around my home")
...
```{r}
route_dir_søndervig <- mb_directions(origin = home,
                       destination = "Søndervig, Badevej 1, 6950, Ringkøbing, Denmark",
                       profile = "cycling",
                           steps = TRUE)
...
mapbox_map %>%
  addPolylines(data = route_dir_søndervig, 
               popup = paste0(
                 "Distance (km): ",
                 round(route_dir$distance, 1), 
                 "<br/>Time (minutes): ",
...
library(sf)
isos <- read_sf("../data/isos4326.shp") #loading in hospitals
```
...
dir.create("data")
library(sf)
write_sf(isos, "data/isos4326.shp") # MAKE basemap to raster to improve visualization 
```
...
isos_proj <- st_transform(isos, 25832) # in previous chunk we made "isos" an sf object
...
For Midtjylland, the list is: (THIS IS A LIE :) )
...
open_cage_token <-  read_file("opencage_my_token.txt")

# set key in the environment
Sys.setenv(OPENCAGE_KEY = open_cage_token)
...
```{r geocode-openc}
b <- oc_forward_df(placename = "Regionshospitalet Grenaa in Grenaa", countrycode = "DK") #den her function er ca det samme som i mapbox
leaflet() %>% 
  addTiles() %>% 
...
# Save object
write_csv(h, "data/midtjylland_hospitals.csv")
```

### Plot geocoded data
Well, the list is not perfect in all instances. Sometimes it gets the hospital , other times it places a marker in the middle of a town. If you want to fix it, provide street addresses and re-geocode. (!!!!!!!, it places the marker in the middle of the city)
```{r h-map, echo = FALSE}
...
#  st_transform(crs = 25832) #note there are NA's in the the data I loaded with my token, but not in Adelas
...
leaflet() %>%
  ...
  addPolygons(data =driving_isos, 
              color = rev(colors[1]),
              fillColor = rev(colors[1]),
              fillOpacity = 0.5, 
              opacity = 0.5, 
              weight = 0.2) %>% 
  addPolygons(data =walking_isos, 
              color = rev(colors[2]),
              fillColor = rev(colors[2]),
              fillOpacity = 0.5, 
              opacity = 0.5, 
              weight = 0.2) %>% 
  addPolygons(data =cycling_isos, 
              color = rev(colors[3]),
              fillColor = rev(colors[3]),
              fillOpacity = 0.5, 
              opacity = 0.5, 
              weight = 0.2)
...
```{r}
#get midtjylland polygon
midtjylland <-  getData("GADM", country= "DNK", level=1) #getting a midtjylland polygon

#midtjylland and our isochrones must be of same CRS
st_crs(midtjylland) == st_crs(driving_isos) #they're the same

#find the areas that does not overlap, 
st_area(midtjylland) - st_area(st_union(driving_isos))  #  walking_isos, cycling_isos


## finding the difference
difference <- st_difference(midtjylland, st_union(iso_driving)) #the output is just a polygon
