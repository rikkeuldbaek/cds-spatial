---
title: "06 - Raster manipulation and analysis (RU)"
...
setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Cognitive Science/6th_semester/spatial_analytics/cds-spatial/Week06")
```
...
elevation <- brick("../data/Aster.tif")
...
*-9999 may be an expression of missing values (NA's)*
...
#plot mounds_bb and mounds
plot(mounds$geometry)
plot(mounds_bb, add =TRUE)

#crop elevation layer by mounds object
elev <- crop(elevation, mounds)

#plot all objects together
plot(elev)
...
survey_proj <- st_transform(survey, st_crs(elev))
st_crs(survey_proj) == st_crs(elev) #TRUE

areas <- st_area(survey) # m2 units
...
plot(elev)
plot(survey_big$geometry, add = TRUE)

# Mask the elev layer with survey_big and save as elevation_mask
elevation_mask <- mask(elev, mask = survey_big) #wow this is pretty cool :) 
...
plot(east_small) #pretty colour change compared to plot(east)
...
#### wehuu, now I aggregated the raster (reduced the raster resolution )
```
...
inMemory(elev)
inMemory(mounds)
inMemory(elevation) #oh ohhh now what? 
...
#crop elevation layer by mounds object
crop_elev <- crop(elevation, prominence)
crop_prom <-crop(prominence, elevation)

# Do the overlay using the above define f function in the `fun` argument. 
elevation_prom_overlay <- overlay(crop_elev, crop_prom, fun = f)
